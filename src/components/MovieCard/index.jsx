...
export default function MovieCard({ poster, title, year, href, className }) {
  return (
    <a
      key={title}
      href={href}
      className={`card position-relative rounded-0 mb-2 text-decoration-none bg-danger h-100 border-0 shadow-sm ${className}`}
    >
      <div className="card-body p-0">
        <img src={poster} alt={title} className="w-100 h-100" />
      </div>
      <div className="w-100 h-100 position-absolute start-0 end-0 top-0 bottom-0 bg-black opacity-25"></div>
